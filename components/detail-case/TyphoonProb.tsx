...
import type { Root, Item, Area } from '@/components/types/detailTyps';
...
export default async function TyphoonProb ({result, url}: {result: Root, url:string}) {
  const MeteoInfosArray = Array.isArray(result.Report.Body.MeteorologicalInfos.MeteorologicalInfo) ? result.Report.Body.MeteorologicalInfos.MeteorologicalInfo[1] : result.Report.Body.MeteorologicalInfos;
  
  // prefecture ごとにまとめる
  const itemsArray = Array.isArray(MeteoInfosArray.Item) ? MeteoInfosArray.Item : [ MeteoInfosArray.Item ];
  const InfoitemsMap = itemsArray.groupToMap((item: Item) => item.Area.Prefecture);
  // [
  //   [ '東京都', [{}, {}, {}, {} ] ],
  //   [ '神奈川県', [{}, {}, {}, {} ] ],
  //   [ '千葉県', [{}, {}, {}, {} ] ],
