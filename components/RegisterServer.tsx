...
export default async function RegisterServer() {
  const earthquakeresponse = await fetch(`${process.env.NEXT_PUBLIC_URL}/data/earthquake-area.json`);
  ...
  const tsunamiresponse = await fetch(`${process.env.NEXT_PUBLIC_URL}/data/tsunami-area.json`);
  ...
  const tyhoonresponse = await fetch(`${process.env.NEXT_PUBLIC_URL}/data/tyhoon-area.json`);
